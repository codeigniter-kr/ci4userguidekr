...
``PHPUnit``\ 에 대한 지원이 내장되어 있으며 프레임워크는 애플리케이션의 모든 측면을 가능한 한 쉽게 테스트할 수 있는 편리한 헬퍼 메소드를 제공합니다.
...
시스템에 Composer가 설치되어 있는지 확인 후 프로젝트 루트(애플리케이션 및 시스템 디렉토리를 포함하는 디렉토리)의 커맨드 라인에 다음을 입력하십시오.
...
애플리케이션을 단위로 테스트하는 경우 ``phpunit.xml``\ 에서 ``system`` 폴더와 ``vendor`` 또는 ``ThirdParty`` 폴더를 제외해야합니다.
