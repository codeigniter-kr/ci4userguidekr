...
이 파일에는 서버별 설정이 포함되어 있습니다. 버전 제어 시스템에 이 중요한 정보를 커밋한 후 저장하지 마세요.
여기에는 이미 가장 일반적인 항목 중 일부가 포함되어 있지만, 모두 주석 처리되어 있습니다.
``production`` 모드 변경을 위해 CI_ENVIRONMENT가 있는 라인의 주석 처리를 제거하고 ``development``\ 로 변경합니다.
...
1. 맨 위에 있는 빨간 머리글 위로 마우스를 가져 가면 Google.com에서 예외를 검색하여 새 탭에 보여주는 ``search`` 링크가 나타납니다.
