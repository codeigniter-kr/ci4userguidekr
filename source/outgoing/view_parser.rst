...
``Parser`` 클래스는 응용 프로그램의 뷰 경로에 저장된 "PHP/HTML 스크립트"\ 를 처리합니다.
...
이는 파서가 사용할 수 있는 asArray 메서드가 있는 Entity 클래스에 특히 유용합니다.
...
주석은 ``{#  #}`` 기호로 묶어 템플리트에 배치할 수 있으며, 파싱(Parsing)하면서 무시되고 제거됩니다.
...
.. note:: 백그라운드에서 **eval()**\ 을 사용하여 조건부 구문을 분석하므로 조건부에서 사용되는 사용자 데이터를 주의하여 관리해야합니다. 그렇지 않으면 보안 위험에 따라 애플리케이션 코드가 노출될 수 있습니다.
...
템플리트에서 참조되지 않은 대체 매개 변수를 포함하면 무시됩니다.
...
템플리트에서 참조되는 대체 매개 변수를 포함하지 않으면 유사 변수 그대로 결과에 표시됩니다.
